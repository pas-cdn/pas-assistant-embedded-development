import { _ as s, k as xt, m as E, l as j, ad as kt, ae as vt, af as _t, ag as bt, W as wt, ah as St, P as Et } from "./mermaid-VLURNSYL-KtsFwY3v.js";
import { d as nt } from "./arc-DZKax0Mz.js";
var Q = function() {
  var n = /* @__PURE__ */ s(function(x, r, a, c) {
    ...
    parse: /* @__PURE__ */ s(function(r) {
      var a = this, c = [0], u = [], y = [null], o = [], w = this.table, v = "", N = 0, H = 0, z = 2, U = 1, P = o.slice.call(arguments, 1), g = Object.create(this.lexer), b = { yy: {} };
...
Expecting ` + O.join(", ") + ", got '" + (this.terminals_[S] || S) + "'" : K = "Parse error on line " + (N + 1) + ": Unexpected " + (S == U ? "end of input" : "'" + (this.terminals_[S] || S) + "'"), this.parseError(K, {
        ...
        switch (I[0]) {
          case 1:
            c.push(S), y.push(g.yytext), o.push(g.yylloc), c.push(I[1]), S = null, H = g.yyleng, v = g.yytext, N = g.yylineno, M = g.yylloc;
            break;
          case 2:
            if ($ = this.productions_[I[1]][1], R.$ = y[y.length - $], R._$ = {
              ...
              return J;
            $ && (c = c.slice(0, -1 * $ * 2), y = y.slice(0, -1 * $), o = o.slice(0, -1 * $)), c.push(this.productions_[I[1]][0]), y.push(R.$), o.push(R._$), et = w[c[c.length - 2]][c[c.length - 1]], c.push(et);
            break;
          case 3:
            return !0;
        }
...
var F = "", lt = 0, X = [], G = [], W = [], ot = /* @__PURE__ */ s(() => St, "getCommonDb"), ct = /* @__PURE__ */ s(function() {
  X.length = 0, G.length = 0, F = "", W.length = 0, Et();
...
var Ft = /* @__PURE__ */ s(function(n, t, e, l) {
  ...
  return t.height = p.height + k * 1.1 * 0.5 + t.padding, t.height = Math.max(t.height, t.maxHeight), t.width = t.width + 2 * t.padding, f.attr("transform", "translate(" + t.width / 2 + ", " + t.padding / 2 + ")"), zt(h, t, i, l), t;
  ...
  for (const [P, g] of k.entries()) {
    ...
    g.events.length > 0 && (M += (g.events.length - 1) * 10), H = Math.max(H, M);
  }
  E.debug("maxSectionHeight before draw", r), E.debug("maxTaskHeight before draw", a), x && x.length > 0 ? x.forEach((P) => {
    const g = k.filter((V) => V.section === P), b = {
      number: w,
      descr: P,
      section: w,
    ...
    const L = p.append("g"), M = C.drawNode(L, b, w, i);
    E.debug("sectionNode output", M), L.attr("transform", `translate(${y}, ${u})`), o += r + 50, g.length > 0 && st(
      p,
      g,
      w,
      y,
  ...
  const z = p.node().getBBox();
  E.debug("bounds", z), _ && p.append("text").text(_).attr("x", z.width / 2 - d).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), c = v ? r + a + 150 : a + 100, p.append("g").attr("class", "lineWrapper").append("line").attr("x1", d).attr("y1", c).attr("x2", z.width + 3 * d).attr("y2", c).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), kt(
    void 0,
    p,
