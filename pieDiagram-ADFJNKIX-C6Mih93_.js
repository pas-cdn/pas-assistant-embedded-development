import { _ as p, h as U, s as J, i as Y, j as Z, J as q, I as H, m as F, k as K, Y as Q, a1 as X, a7 as ee, n as te, P as ae, $ as re } from "./mermaid-VLURNSYL-KtsFwY3v.js";
import { p as ne } from "./chunk-4BX2VUAB-HhHWw7aj.js";
import { p as ie } from "./treemap-KMMF4GRG-Cc2R6JQP.js";
import { d as R } from "./arc-DZKax0Mz.js";
import { b as S, t as z, B as se, C as le } from "./embed-entry-DNbXY835.js";
function oe(e, a) {
...
function ue() {
  ...
  function l(t) {
    var n, c = (t = se(t)).length, d, y, m = 0, u = new Array(c), i = new Array(c), v = +x.apply(this, arguments), w = Math.min(z, Math.max(-z, s.apply(this, arguments) - v)), h, D = Math.min(Math.abs(w) / c, o.apply(this, arguments)), $ = D * (w < 0 ? -1 : 1), g;
    for (n = 0; n < c; ++n)
...
` + e);
  const s = x.db, o = K(), l = Q(s.getConfig(), o.pie), t = 40, n = 18, c = 4, d = 450, y = d, m = X(a), u = m.append("g");
  u.attr("transform", "translate(" + y / 2 + "," + d / 2 + ")");
  const { themeVariables: i } = o;
  let [v] = ee(i.pieOuterStrokeWidth);
  v ??= 2;
  const w = l.textPosition, h = Math.min(y, d) / 2 - t, D = R().innerRadius(0).outerRadius(h), $ = R().innerRadius(h * w).outerRadius(h * w);
  u.append("circle").attr("cx", 0).attr("cy", 0).attr("r", h + v / 2).attr("class", "pieOuterCircle");
  const g = s.getSections(), A = Ce(g), C = [
    i.pie1,
    i.pie2,
    i.pie3,
    i.pie4,
  ...
  const _ = Math.max(
    ...E.selectAll("text").nodes().map((r) => r?.getBoundingClientRect().width ?? 0)
  ), I = y + t + n + c + _;
  m.attr("viewBox", `0 0 ${I} ${d}`), te(m, d, I, l.useMaxWidth);
}, "draw"), $e = { draw: De }, ze = {
  parser: ye,
  db: L,
  renderer: $e,
